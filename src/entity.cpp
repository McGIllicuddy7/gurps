...
void GEntity::on_tick_player(float delta_time) {
        ...
        this->handle_actions(delta_time);
}

void GEntity::on_tick_paused(float delta_time) { return; }
void GEntity::on_render() {
        ...
        DrawLine(m_position.x + (dt.x * radius * weapon_offset),
                 m_position.y + dt.y * (radius * weapon_offset),
                 m_position.x + (dt.x * radius * weapon_offset) +
                     (m_facing.x * radius * weapon_length),
                 m_position.y + (dt.y * radius * weapon_offset) +
                     (m_facing.y * radius * weapon_length),
                 BLACK);
}
GEntity *new_entity(GEntity et) { return get_game()->entities.create(et); }

GBounds GEntity::get_bounds() const {
        return GBounds{.x = int(m_position.x - m_width / 2),
                       .y = (int)(m_position.y - m_height / 2),
                       .width = (int)m_width,
                       .height = (int)m_height};
}
...
void GEntity::handle_movement(float delta_time, g_float2 move_dir) {
        ...
        if (get_game()->check_collision_rect(this->get_bounds())) {
                ...
                if (get_game()->check_collision_rect(this->get_bounds())) {
                        m_position = old_pos;
                        m_position.y +=
                            move_dir.y * delta_time * m_movement_speed / 2.;
                        if (get_game()->check_collision_rect(
                                this->get_bounds())) {
                                m_position = old_pos;
                        }
                }
...
void GEntity::fire_weapon(float delta_time) {
        GRaycastResult rc = get_game()->raycast(
            m_position, m_position + m_facing * 3000, {this});
        if (rc.hit) {
                GParticle p = {};
                p.m_color = GREEN;
                p.m_exists = true;
                ...
                p.m_remaining_lifetime = 0.2;
                spawn_particle(p);
                if (rc.hit_entity) {
                        rc.hit_entity->apply_damage(roll(3),
...
void GEntity::handle_actions(float delta_time) {
        switch (m_action_data.current_action) {
        case GENTITY_ACTION_DEFAULT:
                break;
        case GENTITY_ACTION_RELOAD:
                [[fallthrough]];
        case GENTITY_ACTION_SWITCHING_WEAPONS:
                [[fallthrough]];
        case GENTITY_ACTION_RECOIL:
                m_action_data.remaining_time -= delta_time;
                if (m_action_data.remaining_time < 0.) {
                        m_action_data.remaining_time = 0.0;
                        m_action_data.current_action = GENTITY_ACTION_DEFAULT;
                }
                break;
        }
}
