...
extern constexpr Vector2 to_rl(g_float2 p) { return {p.x, p.y}; }

extern constexpr g_float2 to_game(Vector2 p) { return {p.x, p.y}; }

GWorld *get_world() { return &game.world; }

vector<GEntity *> get_entities() { return game.entities.values(); }
...
void gameloop() {
        ...
        while (!WindowShouldClose()) {
                ...
                [[likely]]
                if (get_game()->game_state == GGAME_STATE_RUNNING ||
                    get_game()->game_state == GGAME_STATE_PAUSED) {
                        game_update();
                        if (get_game()->should_exit) {
                                game_teardown();
                                get_game()->should_exit = false;
                                get_game()->game_state = GGAME_STATE_MENU;
                        }
                } else if (get_game()->game_state == GGAME_STATE_PLAYER_DEAD) {
                        player_dead_update();
                        if (get_game()->should_exit) {
                                game_teardown();
                                get_game()->should_exit = false;
                ...
                } else if (get_game()->game_state == GGAME_STATE_MENU) {
                        menu_update();
                        if (get_game()->should_exit) {
                                menu_teardown();
                                break;
...
void menu_update() { menu_render(); }
void player_dead_update() { player_dead_render(); }

GGame *get_game() { return &game; }
...
void draw_game_world() {
        ...
        get_game()->entities.for_each([&](GEntity &i) {
                if (i.m_is_player) {
                        ...
                        player_pos = Vector2{.x = -i.m_position.x + 500.f,
                                             .y = -i.m_position.y + 400.f};
                        player_facing =
                            Vector2{.x = i.m_facing.x, .y = i.m_facing.y};
                }
        });
        Camera2D cam = {0};
        cam.rotation = 0.;
        ...
        BeginScissorMode(500 - delta * tile_size_px, 400 - delta * tile_size_px,
                         delta * 2 * tile_size_px, delta * 2 * tile_size_px);
        for (int32_t dy = -delta - 1; dy <= delta + 1; dy++) {
                ...
                for (int32_t dx = -delta - 1; dx <= delta + 1; dx++) {
                        ...
                        DrawRectangle((x)*tile_size_px - tile_size_px / 2,
                                      (y)*tile_size_px - tile_size_px / 2,
                                      tile_size_px, tile_size_px, col);
                }
        }
        get_game()->entities.for_each([](GEntity &i) { i.on_render(); });
...
void menu_render() {
        ...
        if (get_gui()->text_button("play game", 40, WHITE)) {
                game.game_state = GGAME_STATE_RUNNING;
...
void player_dead_render() {
        ...
        draw_game_world();
        get_gui()->begin_panel(100, 100, 600, 400);
        get_gui()->text("you died :(", 60, WHITE);
        if (get_gui()->text_button("play again", 40, WHITE)) {
                game_teardown();
                game.game_state = GGAME_STATE_RUNNING;
                get_game()->world = game_generate_world();
                game_setup(nullptr);
        }
        if (get_gui()->text_button("exit to menu", 30, WHITE)) {
                game_teardown();
                game.game_state = GGAME_STATE_MENU;
        }
        get_gui()->end_panel();
        get_gui()->end_frame();
        get_gui()->render();
...
GUI *get_gui() { return &game.gui; }

void game_tick() {
        float delta_time = GetFrameTime();
        if (get_game()->game_state == GGAME_STATE_RUNNING) {
                get_game()->entities.for_each(
                    [=](GEntity &i) { i.on_tick(delta_time); });
                particle_updates(delta_time);
                for (auto &i : get_game()->destroy_queue) {
                        get_game()->entities.destroy(i);
                }
                get_game()->destroy_queue.clear();
        } else if (get_game()->game_state == GGAME_STATE_PAUSED) {
                get_game()->entities.for_each(
                    [=](GEntity &i) { i.on_tick_paused(delta_time); });
        } else {
                assert(false);
        }
}
...
bool GGame::check_collision_rect(GBounds b) {
        ...
        for (int32_t dy = -1; dy <= 1; dy++) {
                for (int32_t dx = -1; dx <= 1; dx++) {
                        GBounds b2 =
                            GBounds{.x = (base_x + dx) * WORLD_TILE_SIZE -
                                         WORLD_TILE_SIZE / 2,
                                    .y = (base_y + dy) * WORLD_TILE_SIZE -
                                         WORLD_TILE_SIZE / 2,
                                    .width = WORLD_TILE_SIZE,
                                    .height = WORLD_TILE_SIZE};
                        Rectangle r1 = {.x = float(b.x),
                                        .y = float(b.y),
                                        .width = float(b.width),
                                        .height = float(b.height)};
                        Rectangle r2 = {.x = float(b2.x),
                                        .y = float(b2.y),
                                        .width = float(b2.width),
                                        .height = float(b2.height)};
                        if (CheckCollisionRecs(r1, r2)) {
                                int32_t bx = base_x + dx;
                                int32_t by = base_y + dy;
                                if (bx < 0 || bx >= WORLD_TILE_DIM || by < 0 ||
                                    by >= WORLD_TILE_DIM) {
                                        return true;
                                }
...
bool check_raycast_rectangle(g_float2 start, g_float2 end, Rectangle r,
                             g_float2 *output, g_float2 *normal) {
...
GRaycastResult GGame::raycast(g_float2 start, g_float2 end,
        ...
        queue.push_back({(int)round(start.x / (WORLD_TILE_SIZE)),
                         (int)round(start.y / (WORLD_TILE_SIZE))});
        ...
        while (!queue.empty()) {
                ...
                for (int32_t dy = -1; dy <= 1; dy++) {
                        for (int32_t dx = -1; dx <= 1; dx++) {
                                ...
                                g_int2 p2 = {x + dx, y + dy};
                                if (reached.find(p2) != reached.end()) {
                                        continue;
                                }
                                if (p2.x < 0 || p2.y < 0 ||
                                    p2.x >= WORLD_TILE_DIM ||
                                ...
                                g_float2 pos = {float(p2.x) * WORLD_TILE_SIZE,
                                                float(p2.y) * WORLD_TILE_SIZE};
                                Rectangle r =
                                    Rectangle{.x = pos.x - WORLD_TILE_SIZE / 2,
                                              .y = pos.y - WORLD_TILE_SIZE / 2,
                                              .width = WORLD_TILE_SIZE,
                                              .height = WORLD_TILE_SIZE};
                                g_float2 tpos;
                                g_float2 tnorm;
                                if (check_raycast_rectangle(start, end, r,
                                                            &tpos, &tnorm)) {
                                        float len =
                                            Vector2Length((tpos - start));
                                        queue.push_back(p2);
                                        reached.insert(p2);
                                        if (len < closest_dist &&
                                            this->world(p2.x, p2.y)
                                                .is_occupied) {
                                                closest = tpos;
        ...
        entities.for_each([&](GEntity &i) {
                ...
                Rectangle r = {float(b.x), float(b.y), float(b.width),
                               float(b.height)};
                g_float2 tpos;
                g_float2 tnorm;
...
void particle_rendering(float delta_time) {
        ...
        for (const GParticle &i : game.particles) {
                if (i.m_exists) {
                        switch (i.m_kind) {
                        case GPARTICLE_LINE: {
                                DrawLine(i.m_pos.x, i.m_pos.y, i.m_pos2.x,
                                         i.m_pos2.y, i.m_color);
                        }
                        case GPARTICLE_PARTICLE: {
                                DrawCircle(i.m_pos.x, i.m_pos.y, i.m_radius,
                                           i.m_color);
                        }
                        case GPARTICLE_SQUARE: {
                                DrawRectanglePro(
                                    Rectangle{i.m_pos.x - i.m_pos.x / 2,
                                              i.m_pos.y - i.m_height / 2},
                                    {0., 0.0}, i.m_rotation, i.m_color);
                        }
                        }
                }
        }
...
void delete_entity(GEntity *et) { get_game()->destroy_queue.push_back(et); }
...
void player_died() { get_game()->game_state = GGAME_STATE_PLAYER_DEAD; }
